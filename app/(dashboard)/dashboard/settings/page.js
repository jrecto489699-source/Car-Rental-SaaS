...
import {
  ...
  Check, Eye, EyeOff, Zap, Globe, Phone, Mail, MapPin
} from 'lucide-react'
  ...
  return (
    <div className="p-6 space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-sm text-slate-400 mt-1">Manage your account and business preferences</p>
      </motion.div>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
          ...
          <nav className="space-y-1">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <Icon size={16} className={activeTab === tab.id ? 'text-blue-400' : 'text-slate-500'} />
                  {tab.label}
                </button>
              )
            })}
        ...
        <div className="flex-1">
          {activeTab === 'company' && (
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="p-6 rounded-2xl border border-white/8 bg-slate-900/60">
              <h2 className="text-base font-semibold text-white mb-6">Company Profile</h2>

              {/* Logo upload */}
              <div className="flex items-center gap-4 mb-6 p-4 rounded-xl bg-slate-800/40 border border-white/8">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center shadow-lg shadow-blue-500/25">
                  <Zap size={28} className="text-white" fill="currentColor" />
                </div>
                <div>
                  <p className="text-sm font-medium text-white mb-1">Company Logo</p>
                  <p className="text-xs text-slate-400 mb-2">PNG, JPG up to 2MB</p>
              ...
              <div className="mt-6 flex justify-end">
                <Button onClick={handleSaveCompany} loading={saving} icon={<Save size={14} />}>
                  Save Changes
                </Button>
              </div>
            </motion.div>
          ...
          {activeTab === 'account' && (
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="p-6 rounded-2xl border border-white/8 bg-slate-900/60">
              <h2 className="text-base font-semibold text-white mb-6">Account Settings</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="p-6 rounded-2xl border border-white/8 bg-slate-900/60">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={key} className="flex items-center justify-between p-4 rounded-xl bg-slate-800/40 border border-white/8">
                    <div>
              ...
              <div className="mt-6 flex justify-end">
                <Button onClick={handleSaveNotifs} loading={saving} icon={<Save size={14} />}>
                  Save Preferences
                </Button>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="p-6 rounded-2xl border border-white/8 bg-slate-900/60">
                  ...
                  <div key={key}>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="••••••••"
                        className="w-full bg-slate-900/60 border border-white/10 rounded-xl text-sm text-white py-3 pl-10 pr-11 focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/15 transition-all"
                      />
                      <button type="button" onClick={() => setShowPw(p => ({ ...p, [key]: !p[key] }))} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white">
                        {showPw[key] ? <EyeOff size={15} /> : <Eye size={15} />}
                      </button>
                    </div>
                  </div>
                ))}
                <Button type="submit" loading={saving} icon={<Lock size={14} />}>
                  Update Password
                </Button>
              </form>
              ...
              <div className="mt-8 pt-6 border-t border-white/8">
                ...
                <div className="p-4 rounded-xl border border-rose-500/20 bg-rose-500/5">
                  ...
                  <button className="text-xs text-rose-400 hover:text-rose-300 border border-rose-500/30 hover:border-rose-500/50 px-3 py-1.5 rounded-lg transition-colors">
                    Delete Account
                  </button>
                </div>
              </div>
